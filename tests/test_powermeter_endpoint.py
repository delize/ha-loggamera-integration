...
def test_power_meter(api_key, device_id):
    ...
    pm_data = {"ApiKey": api_key, "DeviceId": device_id}
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if "Message" in response and response["Message"] == "invalid endpoint":
                ...
                "Error" in response
                and response["Error"]
                and response["Error"].get("Message") == "invalid endpoint"
            ):
                print("❌ PowerMeter endpoint not available")
                response = None
    ...
    if not response:
        ...
        raw_data = {"ApiKey": api_key, "DeviceId": device_id}
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if response and "Data" in response and "Values" in response["Data"]:
                    ...
                    power_value = next(
                        (v for v in values if v.get("ClearTextName") == "Power"), None
                    )
                    if power_value:
                        print(
                    ...
                    energy_value = next(
                        (
                            v
                            for v in values
                            if v.get("ClearTextName") == "Energy imported "
                        ),
                        None,
                    )
                    if energy_value:
                        print(
                            f"Energy value found: {energy_value.get('Value', 'N/A')} {energy_value.get('UnitPresentation', '')}"
                        )
    ...
    if not response:
        ...
        generic_data = {"ApiKey": api_key, "DeviceId": device_id}
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if response and "Data" in response and "Values" in response["Data"]:
                    values = response["Data"]["Values"]
                    print(
                        f"✅ Success! Found {len(values)} values in GenericDevice endpoint:"
                    )
                    for value in values:
                        print(
                            f"  • {value.get('ClearTextName', value.get('Name', 'Unknown'))}: {value.get('Value', 'N/A')} {value.get('UnitPresentation', '')}"
