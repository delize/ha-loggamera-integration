...
def test_power_meter(api_key, device_id):
    ...
    response = make_request(
        "PowerMeter", {"ApiKey": api_key, "DeviceId": device_id, "DateTimeUtc": now}
    )
    ...
    if response and "Data" in response and "Values" in response["Data"]:
        ...
        for value in response["Data"]["Values"]:
            print(
                f"  - {value['ClearTextName']}: {value['Value']} {value['UnitPresentation']}"
            )
    else:
        ...
        raw_response = make_request(
            "RawData", {"ApiKey": api_key, "DeviceId": device_id, "DateTimeUtc": now}
        )
    ...
    cap_response = make_request(
        "GetCapabilities", {"ApiKey": api_key, "DeviceId": device_id}
    )

    if (
        cap_response
        and "Data" in cap_response
        and "Capabilities" in cap_response["Data"]
    ):
    ...
    generic_response = make_request(
        "GenericDevice", {"ApiKey": api_key, "DeviceId": device_id, "DateTimeUtc": now}
    )

    if (
        generic_response
        and "Data" in generic_response
        and "Values" in generic_response["Data"]
    ):
