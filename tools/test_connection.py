...
def make_request(endpoint, data):
    ...
    try:
        ...
        if "CERTIFICATE_VERIFY_FAILED" in error_str:
            logger.error(
                "Certificate verification failed. This could be due to a missing CA certificate."
            )
            ...
            logger.error(
                "Wrong TLS protocol version. The server might not support the TLS version being used."
            )
...
def main():
    ...
    if "Data" in orgs_response and "Organizations" in orgs_response["Data"]:
        ...
        for i, org in enumerate(organizations):
            print(
                f"  {i+1}. {org.get('Name', 'Unknown')} (ID: {org.get('Id', 'Unknown')})"
            )
    ...
    print(
        "You can also try running the diagnose_tls.sh script to diagnose and fix SSL/TLS issues."
    )
