    ...
    if "LogDateTimeUtc" in data:
        ...
        try:
            timestamp = datetime.fromisoformat(
                data["LogDateTimeUtc"].replace("Z", "+00:00")
            )
            now = datetime.utcnow()
            age = now - timestamp
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if "Message" in result and result["Message"] == "invalid endpoint":
                ...
                and result["Error"]["Message"] == "invalid endpoint"
            ):
                print(f"❌ Endpoint not available: {endpoint}")
                return None
            ...
            if "Data" in result and result["Data"] is not None:
                if "Values" in result["Data"]:
                    ...
                    for value in values:
                        print(
                            f"  • {value.get('ClearTextName', value.get('Name', 'Unknown'))}: {value.get('Value', 'N/A')} {value.get('UnitPresentation', '')}"
                        )
                    return values
                elif (
                    "ReadCapabilities" in result["Data"]
                    or "WriteCapabilities" in result["Data"]
                ):
                    read_caps = result["Data"].get("ReadCapabilities", [])
                    write_caps = result["Data"].get("WriteCapabilities", [])
                    print(
                        f"✅ Success! Found {len(read_caps)} read capabilities and {len(write_caps)} write capabilities"
                    )
                    return {
                        "ReadCapabilities": read_caps,
                        "WriteCapabilities": write_caps,
                    }
                else:
        ...
        if response.status_code == 200:
            ...
            if "Data" in result and "Devices" in result["Data"]:
                ...
                for device in devices:
                    print(
                        f"  • {device.get('Title', 'Unnamed')} (ID: {device['Id']}, Type: {device['Class']})"
                    )
                return devices
            else:
...
def main():
    ...
    parser.add_argument(
        "--monitor",
        action="store_true",
        help="Monitor for updates (checks every 5 minutes)",
    )
    parser.add_argument(
        "--verbose",
        "-v",
        action="store_true",
        help="Show verbose output including SSL info",
    )
    ...
    for endpoint in ENDPOINTS:
        ...
        if result and endpoint in ["PowerMeter", "RawData", "GenericDevice"]:
            ...
            url = f"{BASE_URL}/{endpoint}"
            data = {"ApiKey": args.api_key, "DeviceId": device_id}
            response = requests.post(
                url,
                headers={"Content-Type": "application/json"},
                data=json.dumps(data),
                    ...
                    try:
                        ...
                        if "Data" in response and "LogDateTimeUtc" in response["Data"]:
                            ...
                            if endpoint in last_timestamps:
                                ...
                                print(
                                    f"📊 Initial timestamp for {endpoint}: {current_timestamp}"
                                )
                                last_timestamps[endpoint] = current_timestamp
                    except Exception as e:
