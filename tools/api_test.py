...
def test_powermeter(api_key, device_id):
    ...
    try:
        ...
        if "Data" in result and result["Data"] is not None and "Values" in result["Data"]:
...
def test_combined_power_meter(api_key, device_id):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if "Data" in result and result["Data"] is not None and "Values" in result["Data"] and result["Data"]["Values"]:
                power_meter_data = result
                power_meter_values = result["Data"]["Values"]
                print(f"   Success! Got {len(power_meter_values)} values from PowerMeter endpoint")
                ...
                if response.status_code == 200:
                    ...
                    if "Data" in result and result["Data"] is not None and "Values" in result["Data"] and result["Data"]["Values"]:
                        power_meter_data = result
                        power_meter_values = result["Data"]["Values"]
                        print(f"   Success! Got {len(power_meter_values)} values from PowerMeter endpoint with DateTimeUtc")
                        ...
                        print("   No values found in PowerMeter response with DateTimeUtc")
        else:
    ...
    if not power_meter_values:
        ...
        if synthetic_count > 0:
            print(f"   Created {synthetic_count} synthetic PowerMeter values from RawData")
    
    # Add RawData values that don't already exist in the combined set
    existing_names = [v.get("Name") for v in combined_values]
    existing_cleartext = [v.get("ClearTextName") for v in combined_values if v.get("ClearTextName")]
    ...
    for value in combined_values:
        ...
        elif name in ["ConsumedTotalInkWh", "PowerInkW", "alarmActive", "alarmInClearText"]:
            source = "PowerMeter"
        else:
            source = "RawData"
        ...
        if name in ["ConsumedTotalInkWh", "PowerInkW", "544352", "544399", "550205", "550206", "550207"]:
...
def main():
    ...
    parser.add_argument("device_id", nargs="?", help="Device ID for device-specific tests")
    parser.add_argument("--org-id", help="Organization ID")
    parser.add_argument("--test", choices=["organizations", "powermeter", "devices", "raw", "combined", "all"], 
    ...
    if args.test in ["powermeter", "raw", "combined", "all"] and not args.device_id:
        print("Error: device_id is required for PowerMeter, RawData, and Combined tests")
