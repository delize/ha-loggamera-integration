...
def print_response(response, verbose=True):
    ...
    for value in values:
        ...
        print(
            f"  {clear_name}: {value.get('Value', 'N/A')} {unit} ({name}, {value_type})"
        )
...
def poll_powermeter(api_key, device_id, interval, count=None):
    ...
    try:
        while True:
            ...
            if polls > 1:
                print(
                    f"Stats: {updates} updates in {polls} polls ({updates/polls*100:.1f}%)"
                )

                if updates > 0:
                    print(
                        f"Average: 1 update per {polls/updates:.1f} polls (approx. {polls/updates*interval/60:.1f} minutes)"
                    )
        ...
        print(
            f"Summary: {updates} updates in {polls} polls over {polls*interval/60:.1f} minutes"
        )
        if updates > 0:
            print(
                f"Average update frequency: approximately every {polls/updates*interval/60:.1f} minutes"
            )
...
def main():
    ...
    parser.add_argument(
        "--device-id", required=True, type=int, help="Device ID to query"
    )
    parser.add_argument(
        "--poll",
        type=int,
        help="Poll interval in seconds (if not provided, only polls once)",
    )
    parser.add_argument(
        "--count",
        type=int,
        help="Number of polls to perform (if not provided, polls indefinitely)",
    )
